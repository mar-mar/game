...
export default class GameAPI extends CanvasAPI {
  ...
  private async removeGemLines(): Promise<boolean> {
    ...
    // прибавляем очки
    this.counts += gems.length

    await this.removeGemLines()
    return true
  }
  ...
  public async trySelectedGem(
    ...
    if (sideBySide) {
      ...
      const success = await this.removeGemLines() // если success===true, внутри скинется select
